...
(define-struct flappy(x y yspeed))

(define-struct tube(x ygap))

(define-struct varb(grav moment scroll))

(define-struct puncte(x y val))

(define-struct gamestate(flappy listtube varb puncte))
...
(define (get-initial-state)
  (define pasare(flappy bird-x bird-initial-y 0))
  (define teava (tube scene-width (+ added-number (random random-threshold))))
  (define var(varb initial-gravity initial-momentum initial-scroll-speed))
  (define scor(puncte text-x text-y 0))
  (define initial-state (gamestate pasare (list teava) var scor))
  initial-state)
...
(define (next-state-bird bird gravity)
  (struct-copy flappy bird [y (+ (flappy-y bird) (flappy-yspeed bird))] [yspeed (+ (flappy-yspeed bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy flappy bird [yspeed (- (flappy-yspeed bird) momentum)]))
...
(define (change current-state pressed-key)
  (cond
    ((key=? pressed-key " ") (struct-copy gamestate current-state [flappy (next-state-bird-onspace (gamestate-flappy current-state) initial-momentum)]))
    (else current-state)))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (z) (struct-copy tube z [x (- (tube-x z) scroll-speed)])) pipes))
...
(define (clean-pipes pipes)
  (cond
    ((null? pipes) null)
    ((> 0 (+ (tube-x (car pipes)) pipe-width)) (clean-pipes (cdr pipes)))
    (else (append (list (car pipes)) (clean-pipes (cdr pipes))))))
;(filter (lambda (x) (< 0 (+ (tube-x x) pipe-width))) pipes))
;TODO 13
...
(define (add-more-pipes pipes)
  (if (> no-pipes (length pipes))
      (append pipes (list (tube (+ (+ pipe-width pipe-gap) (tube-x (last pipes))) (+ added-number (random random-threshold)))))
      pipes))
;TODO 14
...
(define (get-score state)
  (puncte-val (gamestate-puncte state)))
...
(define (check-ground-collision bird)
 (if (>= (+ (flappy-y bird) bird-height) ground-y)
     #t
     #f))
...
(define (invalid-state? state)
  (or (check-pipe-collisions (gamestate-flappy state) (gamestate-listtube state)) (check-ground-collision (gamestate-flappy state))))
...
(define (check-pipe-collisions bird pipes)
  (foldl (lambda (v result)
         (or (check-collision-rectangles (make-posn (flappy-x bird) (flappy-y bird)) (make-posn (+ (flappy-x bird) bird-width) (+ (flappy-y bird) bird-height))
                                          (make-posn (tube-x v) 0) (make-posn (+ (tube-x v) pipe-width) (tube-ygap v)))
             (check-collision-rectangles (make-posn (flappy-x bird) (flappy-y bird)) (make-posn (+ (flappy-x bird) bird-width) (+ (flappy-y bird) bird-height))
                                          (make-posn (tube-x v) (+ (tube-ygap v) pipe-self-gap)) (make-posn (+ (tube-x v) pipe-width) ground-y))
              result))
         #f pipes))
...
(define (next-state state)
  (struct-copy gamestate state
               [flappy (next-state-bird (gamestate-flappy state) (varb-grav (gamestate-varb state)))]
               [listtube (next-state-pipes (gamestate-listtube state) (varb-scroll (gamestate-varb state)))]
               [puncte (puncte text-x text-y (+ (puncte-val (gamestate-puncte state)) 0.1))]))
...
(define (draw-frame state)
  (match-let ([(gamestate flappy listtube varb puncte) state])
    (place-image bird-image (+ (flappy-x flappy) (quotient bird-width 2))
                            (+ (flappy-y flappy) (quotient bird-height 2))
                            (place-image ground-image (quotient scene-width 2)
                                         (+ (quotient ground-height 2) ground-y)
                                         (place-image (score-to-image (puncte-val puncte)) (puncte-x puncte) (puncte-y puncte) (place-pipes listtube initial-scene))))))
    ;(place-pipes listtube initial-scene)))
    
; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (if (null? pipes)
      scene
      (place-pipes (cdr pipes) (place-image (rectangle (add1 pipe-width) pipe-self-gap "solid" "white")
                                            (+ (tube-x (car pipes)) (quotient pipe-width 2))
                                            (+ (tube-ygap (car pipes)) (quotient pipe-self-gap 2)) (place-image (rectangle pipe-width (+ pipe-height pipe-height)  "solid" "green")
                                                                                                                (+ (tube-x (car pipes)) (quotient pipe-width 2))
                                                                                                                (+ (tube-ygap (car pipes)) (quotient pipe-self-gap 2)) scene)))))
  ;(let itr ((pipes pipes))
   ; (place-image (rectangle pipe-width (tube-ygap (car pipes))  "solid" "green")
    ;             (+ (tube-x (car pipes)) (quotient pipe-width 2))
                 ;(+ (+ (tube-ygap (car pipes)) pipe-self-gap) (quotient (- ground-y (+ (tube-ygap (car pipes)) pipe-self-gap)) 2))
     ;(place-image (rectangle pipe-width (tube-ygap (car pipes))  "solid" "green")
      ;            (/ (+ (+ (tube-x (car pipes)) (tube-x (car pipes))) pipe-width) 2)
       ;           (quotient (tube-ygap (car pipes)) 2) scene))))
